  ...
  private async refreshLatest(): Promise<void> {
    try {
      ...
      const newLatest = await this.getPrompt({
        minorVersion: PromptSpecialVersion.LATEST,
      ...
      const oldLatest = this.prompts[PromptSpecialVersion.LATEST];

      // Update the prompt
      this.prompts[PromptSpecialVersion.LATEST] = newLatest;
...
function makeMinorVersionsToRequest(args: {
  ...
  if (args.majorVersion === PromptSpecialVersion.DANGEROUSLY_USE_UNDEPLOYED) {
