...
// existing content`;
    ...
    it('should handle file without APP_NAME_TO_ID declaration', async () => {
      const mockContent = `// existing content
interface __Autogenerated__PromptsV2Types {}
// existing content`;
      (fs.access as jest.Mock).mockResolvedValueOnce(undefined);
  ...
  describe('runV2', () => {
    ...
    it('should throw error if API key is not set', async () => {
      const readEnvSpy = jest.spyOn(util, 'readEnv');
      readEnvSpy.mockReturnValue(undefined);

      await expect(runV2()).rejects.toThrow(
        `You must set the ${util.AutoblocksEnvVar.AUTOBLOCKS_V2_API_KEY} environment variable`,
      );
    });

    it('should handle empty prompts array', async () => {
      const readEnvSpy = jest.spyOn(util, 'readEnv');
      readEnvSpy.mockReturnValue('test-api-key');
